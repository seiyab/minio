...
func handleKMSConfig() {
	...
	if _, err = KMS.GenerateKey(GlobalContext, &kms.GenerateKeyRequest{}); errors.Is(err, kms.ErrKeyNotFound) {
		if err = KMS.CreateKey(GlobalContext, &kms.CreateKeyRequest{
			Name: KMS.DefaultKey,
		}); err != nil && !errors.Is(err, kms.ErrKeyExists) && !errors.Is(err, kms.ErrPermission) {
			logger.Fatal(err, "Failed to connect to KMS: failed to create server-side encryption default key")
		}
	}
	if err != nil {
		logger.Fatal(err, "Failed to connect to KMS")
